...
| Caddy binary features (plugins, Go/Caddy version) | `saas-gateway/Dockerfile` (+ `caddy-domainlookup`, `certmagic-s3`) | `cvhome/store-pod/spg/Dockerfile` FROM pin, `cvhome/docker-compose-lcl.yml` image pin | no — pins are manual and currently differ (`sha-8eed986` vs `sha-4a6d381`) |
...
**Rebuild the Caddy edge**: `caddy-domainlookup` (if plugin change) → push `saas-gateway` main (publishes
`sha-<short>`) → mirror to public ECR (outside these repos; verify the tag exists) → bump both pins in `cvhome`
→ QA `spg` locally (`qa/lcl-qa.md`, custom-domain flow) → nothing to do in `cvhome-platform` (image is
built from the app repo by CodeBuild).
