...
- **Releases are cut here, never in a sub-repo, and they deploy nothing.** `docs/releasing.md` is the runbook,
  `docs/release-plan.md` the design: `Release` tags every `tag: true` repo with the same version and writes
  `releases/vX.Y.Z.yaml` as the compatibility record. Never push a `v*` tag or bump a version file by hand.
