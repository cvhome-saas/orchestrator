...
RULES: list[tuple[str, str, list[str], list[str], list[str]]] = [
    ...
    ("cvhome", r"docker-compose-(lcl|load)\.yml$|extra/scripts/configure-domain\.sh$",
     ["load-testing (TARGET=lcl hostnames, store ids, network aliases)", "cvhome-platform (only if the same env keys exist there)", "saas-gateway (image pin)"],
     ["spg-image", "load-testing"],
     ["Hostnames/aliases/store ids changed? lcl.json and seed data in load-testing must follow.",
      "spg image tag changed here? The Dockerfile pin is the one AWS runs; keep both on the same sha or say why not.",
      "New monitoring service or port? extra/monitoring docs and the load-testing prometheusUrl/grafanaUrl."]),
    ...
    ("cvhome", r"extra/monitoring/.*|.*logging-otel-collector-config\.yml$",
     ["aws-otel-collector/otel-config.yaml", "load-testing/docs/prometheus.md + grafana dashboards", "cvhome-platform flavours.yaml monitoring"],
     ["slo"],
     ["Kept/dropped metrics changed locally? Mirror the decision in aws-otel-collector or document the difference.",
      "Dashboard queries assume metric names/labels that load-testing tags produce."]),
    ...
    ("aws-otel-collector", r"^otel-config\.yaml$|^Dockerfile$",
     ["cvhome-platform services.yaml infra.otel-collector (ports 4317/4318, memory size)", "cvhome common-config.yml otel block (protocol, kept metrics)", "cvhome extra/monitoring (local counterpart)", "load-testing/docs/prometheus.md"],
     ["env"],
    ...
    ("cvhome-saas.github.io", r"^docs/.*",
     ...
     [],
     ["Every service, repo and URL named on the page exists today (see deprecated-repos.md)."]),
    ("ideation", r".*",
