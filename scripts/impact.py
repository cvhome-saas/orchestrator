...
RULES: list[tuple[str, str, list[str], list[str], list[str]]] = [
    ...
    ("cvhome", r"docker-compose-lcl\.yml$|extra/scripts/configure-domain\.sh$",
     ["load-testing/stack/docker-compose.yml (aliases, ports, spg env, postgres/minio image)", "load-testing/k6/config/env/local.json + k6/data seed",
      "saas-gateway (image pin)", "cvhome store-commons/test-support containers (same postgres/minio image)"],
     ["spg-image", "load-testing", "catalog", "infra-images"],
     ["Hostnames/aliases/store ids/spg env changed? load-testing's compose stack and local.json must follow.",
      ...
      "postgres/MinIO image or registry changed? Testcontainers and load-testing/stack pin the same one; move all three together.",
      "Adding a monitoring service here is wrong: the monitoring stack lives in load-testing/stack (decision 2026-09-08)."]),
    ("cvhome", r"store-commons/test-support/.*/containers/.*TestConfiguration\.java$",
     ["cvhome docker-compose-lcl.yml (same postgres/minio image)", "load-testing/stack/docker-compose.yml (same postgres/minio image)"],
     ["infra-images"],
     ["Image or registry changed? cvhome's compose and load-testing's stack pin the same one; a registry that stops serving it breaks CI and `make stack-up` on the first clean pull."]),
    ("cvhome", r"store-commons/commons/src/main/java/com/asrevo/cvhome/commons/domain/Sku\.java$"
               r"|store-core/console-ui/src/app/(models/products|features/product-form/.*)\.ts$",
     ["cvhome console-ui SKU_PATTERN (product form, variants step)", "load-testing/k6/data seed skus + k6/lib/fixtures (K6-SKU-…, K6-EDIT-…)"],
     ["sku-format"],
     ["Sku.FORMAT changed? console-ui's SKU_PATTERN must say the same, or the form passes a sku the save refuses.",
      "Narrowed the rule? Rows already stored fail to load through SkuConverter: run the plan's deploy-note query first, and check the k6 generators."]),
    ...
    ("load-testing", r"^stack/docker-compose\.yml$|^stack/stack\.sh$",
     ...
     ["catalog", "env", "public-ecr", "infra-images"],
     ["Every service/port/alias still matches common-config.yml; a new service in cvhome needs a container here.",
      ...
      "postgres/minio image the same as cvhome's docker-compose-lcl.yml and Testcontainers, on a registry that still serves it?",
    ...
    ("load-testing", r"^k6/config/env/.*\.json$|^k6/data/.*",
     ["cvhome lcl.yml / configure-domain.sh / docker-compose-load.yml (hosts, store ids)", "cvhome Sku.FORMAT (seed skus)"],
     ["load-testing", "sku-format"],
     ["Store ids, hosts, accounts must exist in the app's seed data; only lcl.json and aws.example.json are committed.",
      "Every seed sku matches Sku.FORMAT, or the server refuses it with a 400."]),
    ...
    ("assets", r"^fast-run/.*",
     ...
     ["infra-images"],
     ["Every image, host and service in the script exists today (and its registry still serves it); the raw URL goes live on merge."]),
    ("dot-github", r".*",
